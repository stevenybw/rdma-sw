...
inline int pa_get_successive_rank(YMPID_Topology* topo, int src_pa, int dst_pa, int iter)
{
  int src_prefix = GET_PREFIX(topo, src_pa, iter);
  int dst_prefix = GET_PREFIX(topo, dst_pa, iter);
  if(src_prefix != dst_prefix) {
    return -1;
  }
  int dst_middle = GET_MIDDLE(topo, dst_pa, iter);
  int src_postfix = GET_POSTFIX(topo, src_pa, iter);
  int next_pa = src_prefix + dst_middle + src_postfix;
  assert(next_pa >= 0);
  assert(next_pa <= max_pa);
}
