...
static YMPID_Topology* YMPID_Get_topology(MPI_Comm comm, MPI_Comm* out_comm)
{
  ...
  int num_board=0;
  {
    ...
    sizeof_board[num_board]     = nprocs - begin;
    num_board++;
  }
  ...
  Factorized num_board_factorized;
  {
    ...
    board_size_factorized           = find_largest_no_larger_than(board_size/4);
    board_size_factorized.number    *= 4;
    board_size_factorized.factor.n0 += 2;
    ...
    target_num_board      = num_board_factorized.number;
    np                    = target_num_board * board_size;
  ...
  int selected = 0;
  {
    ...
    for(i=1; i<nprocs; i++) {
      ...
      if(board_id != last_board_id) {
        ...
        if(current_board_size >= board_size) {
          ...
          current_num_board++;
          if(current_num_board == target_num_board) {
            break;
          }
        }
        begin                   = i;
    ...
    if(current_num_board < target_num_board) {
      int current_board_size    = nprocs - begin;
      if(current_board_size >= board_size) {
        int j;
        for(j=begin; j<begin+board_size; j++) {
          if(cpuid_list[j] == my_cpuid) {
    ...
    if(current_num_board != target_num_board) {
      printf("ERROR: current_num_board=%d   target_num_board=%d\n", current_num_board, target_num_board);
      exit(-1);
    }
  }
...
static int iter_group_base[MAX_NUM_ITER];
...
#define GEN_ITERS_INTER_BOARD(FACTOR_ID) do{                                                                                            \
      for(i=0; i<num_board_factorized.factor.n ## FACTOR_ID; i++) {                                                                     \
        ...
        iter_group_base[total_iter] = group_base * board_size;                                                                                       \
...
#define GEN_ITERS_INTRA_BOARD(FACTOR_ID) do{                                                                                            \
      for(i=0; i<board_size_factorized.factor.n ## FACTOR_ID; i++) {                                                                    \
        int group_size = valid_factor[FACTOR_ID];                                                                                       \
        iter_group_size[total_iter] = group_size;                                                                                       \
        iter_group_base[total_iter] = group_base;                                                                                       \
        for(p=0; p<group_size; p++) {                                                                                                   \
  ...
  }
  {
    ...
    int i;
    LOGDS("  Route Table\n");
    LOGDS("%16s%16s%16s\n", "ITER", "GROUP_BASE", "GROUP_SIZE");
    for(i=0; i<total_iter; i++) {
      LOGDS("%16d%16d%16d\n", i, iter_group_base[i], iter_group_size[i]);
    }
  }
  {
...
int main(void)
{
  ...
  YMPI_Shuffle(input, 256, rst, &rst_len);
  MPI_Finalize();
  return 0;
}
